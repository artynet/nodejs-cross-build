#!/bin/bash -x
## Node.js for mipsel MUSL OpenWRT Packaging Script
## =========================================
## Use like this:
...
clean () {
    ...
    rm -rvf node-v$1/ node-v$1-mipsel-openwrt-musl/ node-v$1-linux/
}
...
crossbuild () {
    ...
    cd node-v${1}-mipsel-openwrt-musl/
    ...
    sed -i -e s/small-icu/none/g Makefile
    sed -i -e "s/-\$(ARCH)/\-mipsel-openwrt-musl/g" Makefile
    time make -j3 binary
...
then
    # preparing sources
    untar ${1} mipsel-openwrt-musl
    echo -e "\nFixing v8.gyp file...\n"
    fixv8gyp ${1} mipsel-openwrt-musl
    echo $var
    sleep 1
...
# exporting compilers
export STAGING_DIR=$HOME/x-tools/mipsel-openwrt-linux-musl
...
mv $CURDIR/node-v${1}-mipsel-openwrt-musl/*.tar.* $CURDIR/TARBALL
