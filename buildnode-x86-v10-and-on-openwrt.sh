...
export PATH=$HOME/x-tools/i486-openwrt-linux-musl/bin:$PATH

# raspberry pi 2 cross-compile exports
export HOST="i486-openwrt-linux-musl"
...
mv node-v${1}/ node-v${1}-x86-openwrt/

cd node-v${1}-x86-openwrt/
...
export CONFIG_FLAGS="--without-snapshot --with-intl=none" LDFLAGS="${LDFLAGS} -latomic"
sed -i -e s/small-icu/none/g Makefile
sed -i -e "s/-\$(ARCH)/\-x86-openwrt/g" Makefile
make -j3 binary
